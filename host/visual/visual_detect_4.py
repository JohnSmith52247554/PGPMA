...
# 圆形合并参数
MERGE_CIRCLE_DISTANCE = 30  # 圆形合并距离阈值
MERGE_RADIUS_RATIO = 0.7    # 半径比例阈值

class CircleProcessor:
    """圆形处理工具类"""

    def __init__(self):
        self.previous_circles = None

    def process_circles(self, circles, sort_method='left-to-right'):
        """
        完整的圆形处理流程
        """
        if circles is None:
        ...
        # 过滤圆形
        filtered = self.filter_circles(circles)

        # 合并接近的圆形
        merged = self.merge_close_circles(filtered)

        # 排序圆形
        sorted_circles = self.sort_circles(merged, method=sort_method)

        # 保存当前结果用于下一次跟踪
        self.previous_circles = sorted_circles

        return sorted_circles

    def filter_circles(self, circles, min_radius=5, max_radius=100):
        """过滤掉不符合条件的圆形"""
        if circles is None:
            return None

        circles = np.array(circles)
        if circles.ndim == 3:  # 霍夫圆检测的原始格式
            circles = circles[0]
    ...
    def merge_close_circles(self, circles, distance_thresh=MERGE_CIRCLE_DISTANCE,
                            radius_ratio=MERGE_RADIUS_RATIO):
        """合并接近的圆形"""
        if circles is None or len(circles) < 2:
            return circles

        # 转换为列表形式处理
        circles_list = [{'x': c[0], 'y': c[1], 'r': c[2], 'index': i}
                        for i, c in enumerate(circles)]

        groups = []
        visited = [False] * len(circles_list)
        ...
        for i in range(len(circles_list)):
            ...
            for j in range(i + 1, len(circles_list)):
                ...
                # 计算圆心距离
                dist = np.sqrt((circle_i['x'] - circle_j['x']) ** 2 +
                               (circle_i['y'] - circle_j['y']) ** 2)

                # 计算半径比例
                ratio = min(circle_i['r'], circle_j['r']) / max(circle_i['r'], circle_j['r'])

                # 判断是否应该合并
                if dist < distance_thresh and ratio > radius_ratio:
                    group.append(j)
                    visited[j] = True
        ...
        for group in groups:
            if len(group) == 1:
                # 单个圆，直接保留
                c = circles_list[group[0]]
                merged.append([c['x'], c['y'], c['r']])
            else:
                # 合并多个圆
                group_circles = [circles_list[i] for i in group]

                # 计算合并后的圆心（取平均值）
                center_x = np.mean([c['x'] for c in group_circles])
                center_y = np.mean([c['y'] for c in group_circles])

                # 计算合并后的半径（面积加权平均）
                areas = [np.pi * c['r'] ** 2 for c in group_circles]
                total_area = np.sum(areas)
                merged_r = np.sqrt(total_area / np.pi)

                merged.append([center_x, center_y, merged_r])

        return np.array(merged) if merged else None

    def sort_circles(self, circles, method='left-to-right'):
        """对圆形进行排序"""
        if circles is None:
            return None
        ...
        if method == 'left-to-right':
            ...
            # 按y坐标排序
            return circles[np.argsort(circles[:, 1])]
        elif method == 'radius':
            # 按半径排序
            return circles[np.argsort(circles[:, 2])]
        elif method == 'center-distance':
            # 按到图像中心的距离排序
            ...
            cx, cy = w // 2, h // 2
            dists = [np.sqrt((c[0] - cx) ** 2 + (c[1] - cy) ** 2) for c in circles]
            return circles[np.argsort(dists)]
        else:
            return circles
...
class AverageCalculator:
    """平均值计算器类，用于定期计算位置和旋转的平均值"""

    def __init__(self, update_interval=1.0, history_size=30):
    ...
    def add_data(self, position=None, rotation=None):
    ...
    def _calculate_averages(self):
        if self.position_history:
            self.avg_x = sum(pos[0] for pos in self.position_history) / len(self.position_history)
            self.avg_y = sum(pos[1] for pos in self.position_history) / len(self.position_history)
        if self.rotation_history:
            self.avg_rotation = sum(self.rotation_history) / len(self.rotation_history)

    def get_averages(self):
        has_data = self.has_new_data
        if self.has_new_data:
            self.has_new_data = False
        return self.avg_x, self.avg_y, self.avg_rotation, has_data
...
def find_red_button(frame, console : VisualConsole):
    """检测红色圆形按钮，如果已经检测到则返回存储的坐标"""
    ...
    processed_circles = None
    if circles is not None:
        # 使用圆形处理器处理结果
        processor = CircleProcessor()
        processor.image_shape = img.shape
        processed_circles = processor.process_circles(circles)

        # 绘制处理后的圆形
        if processed_circles is not None:
            for (x, y, r) in processed_circles:
            ...
            if len(processed_circles) == 2:
                with CIRCLES_LOCK:
                    DETECTED_CIRCLES = processed_circles
                # print(f"检测到两个红色圆形，坐标已存储: {processed_circles}")
    else:
        pass
        # print("未检测到红色圆形")

    return img, processed_circles


def calculate_middle_position(circles, frame, px_to_mm=PX_TO_MM):
    ...
    for (x, y, r) in circles:
        offset_x = x - frame_width // 2
        offset_y = y - frame_height // 2
        mm_x.append(offset_x * px_to_mm)
        mm_y.append(offset_y * px_to_mm)

    center_x = (mm_x[0] + mm_x[1]) / 2
    center_y = (mm_y[0] + mm_y[1]) / 2

    return center_x, center_y


def find_rotation(circles):
    if circles is None or len(circles) != 2:
        return None
    ...
    dx = pt1[0] - pt0[0]
    dy = pt1[1] - pt0[1]
    rotation = -(np.degrees(np.arctan2(dy, dx))) % 180

    return rotation


def axis_reload(x, y, rot):
    global AXIS_X, AXIS_Y, ROT

    n_rot = rot - ROT

    rot = math.radians(rot)

    d = -y * math.cos(rot) - x * math.sin(rot)
    x1 = -d * math.sin(rot)
    y1 = -d * math.cos(rot)
    r = math.sqrt((x - x1) ** 2 + (y - y1) ** 2)

    n_x = AXIS_X * math.cos(rot) - AXIS_Y * math.sin(rot) - r
    n_y = AXIS_X * math.sin(rot) + AXIS_Y * math.cos(rot) + d

    return n_x, n_y, n_rot


def axis_reload_2(x, y, rot):
    n_rot = rot - ROT
    rot = math.radians(rot)
    n_x = math.cos(rot)*(AXIS_X-x)+math.sin(rot)*(AXIS_Y-y)
    n_y = math.sin(-rot)*(AXIS_X-x)+math.cos(rot)*(AXIS_Y-y)
    return n_x, n_y, n_rot


def process_frame(frame, avg_calculator, console : VisualConsole):
    """处理单帧图像并更新平均值计算器"""
    processed_frame, circles = find_red_button(frame, console)
    position_result = calculate_middle_position(circles, frame)
    rotation_result = find_rotation(circles)

    # 添加数据到计算器
    avg_calculator.add_data(position=position_result, rotation=rotation_result)

    new_coord = None

    # 更新平均值（如果需要）
    if avg_calculator.update_averages():
        avg_x, avg_y, avg_rotation, has_new_data = avg_calculator.get_averages()

        if has_new_data:
            #print(f"\n=== 1秒平均值更新 ===")
            if avg_calculator.position_history:
                pass
                #print(f"平均中心位置: ({avg_x:.2f}mm, {avg_y:.2f}mm)")
                #print(f"位置数据点数: {len(avg_calculator.position_history)}")

            if avg_calculator.rotation_history:
                pass
                #print(f"平均旋转角度: {avg_rotation:.2f}°")
                #print(f"旋转数据点数: {len(avg_calculator.rotation_history)}")

            if avg_calculator.position_history and avg_calculator.rotation_history:
                x, y, rot = axis_reload_2(avg_x, avg_y, avg_rotation)
                #print(f"真实旋转角度: {rot:.2f}°")
                #print(f"真实中心位置: ({x:.2f}mm, {y:.2f}mm)")
            ...
            #print("===================\n")
            avg_calculator.clear_history()

    # 实时显示当前帧的结果
    if position_result is not None:
        cv2.putText(processed_frame, f"X: {position_result[0]:.1f}mm", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv2.putText(processed_frame, f"Y: {position_result[1]:.1f}mm", (10, 60),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

    if rotation_result is not None:
        cv2.putText(processed_frame, f"Rot: {rotation_result:.1f}°", (10, 90),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

    return processed_frame, new_coord


def reset_red_button_detection():
    """重置红色圆形检测，允许重新检测"""
    global DETECTED_CIRCLES
    with CIRCLES_LOCK:
        DETECTED_CIRCLES = None
...
def detect_and_decode_qrcode(image):
    """改进的QR码检测和解码函数（支持GPU加速），确保所有结果都包含多边形顶点信息"""
    if image is None:
        return []

    # 获取多层次处理图像
    processed_images = preprocess_for_qr_detection(image)
    if processed_images is None:
        return []
    ...
    # 尝试在所有处理后的图像上解码
    for idx, proc_img in enumerate(processed_images.values()):
        if proc_img is None:
            continue
        ...
        for barcode in barcodes:
            ...
            if hasattr(barcode, 'polygon') and barcode.polygon:
                # 如果pyzbar提供了多边形顶点，直接使用
                qr_polygon = np.array([[p.x, p.y] for p in barcode.polygon], dtype=np.float32)
            else:
                # 否则从rect生成一个矩形多边形
                qr_polygon = np.array([
                    [x, y],
            ...
            if qr_id not in existing_ids:
                results.append({
                    "data": barcode_data,
                    "type": barcode_type,
                    "position": (x, y, w, h),
                    "polygon": qr_polygon,  # 确保所有结果都有多边形顶点
                    "id": qr_id,
                    "direction": direction,
                    "secure": secure,
                    "source": idx,  # 记录是从哪种处理图像中检测到的
                    "detection_type": "standard_qr"  # 标记为标准QR码检测
                })
    ...
    for idx, proc_img in enumerate(processed_images.values()):
        ...
        for cnt in contours:
            ...
            if 1000 < area < 50000:  # 面积过滤
                ...
                if approx is None or len(approx) != 4:  # 四边形轮廓
                    continue
                ...
                try:
                    ...
                    for d in decoded:
                        ...
                        qr_id, direction, secure = parse_qr_data(qr_data)
                        ...
                        if qr_id not in existing_ids:
                            ...
                            results.append({
                                "data": qr_data,
                                "type": d.type,
                                "position": (box[0][0], box[0][1], width, height),
                                "polygon": qr_polygon,  # 添加多边形顶点信息
                                "rect": box,
                                "warped": warped,
                                "id": qr_id,
                                "direction": direction,
                                "secure": secure,
                                "source": idx,  # 记录是从哪种处理图像中检测到的
                                "detection_type": "tilted_qr"  # 标记为倾斜QR码检测
                            })
                except:
                    continue
...
def update_coordinate_history() -> dict | None:
    """更新坐标历史记录并计算一秒内的位置平均值"""
    ...
    with COORD_LOCK:
        ...
        if detected:
            ...
            # 移除超过1秒的旧数据
            while COORDINATE_HISTORY and current_time - COORDINATE_HISTORY[0]['time'] > 1.0:
                COORDINATE_HISTORY.popleft()
...
def visual_detect_4(camera_id : int, console : VisualConsole):
    ...
    console.add_message("程序启动")

    # 初始化摄像头
    cap = cv2.VideoCapture(camera_id)
    if not cap.isOpened():
        # logger.error("无法打开摄像头")
        console.add_message("无法打开摄像头")
        return
    ...
    # 修复：在循环外初始化 AverageCalculator
    avg_calculator = AverageCalculator(update_interval=1.0, history_size=30)
        ...
        while True:
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                ...
                try:
                    ...
                    console.add_message(f"并行处理超时: {str(e)}")
                    continue
            ...
            if ZOOM_ENABLED and not all_qr_results:
                ...
                if current_zoom_rect is not None:
                    try:
                        ...
                        console.add_message(f"放大区域搜索失败: {str(e)}")
            ...
            if not all_qr_results and not should_keep_priority_lock(current_time):
                with COORD_LOCK:
                    DETECTED = False
                    AXIS_X = 0
                    AXIS_Y = 0
            ...
            if all_qr_results:
                ...
                display_qr = last_valid_qr.get("polygon")
                if display_qr is not None:
                    center, mm_x, mm_y, mm_z, rotation = calculate_qr_position(
                        display_qr, frame.shape[1], frame.shape[0])

                    # 检查返回值是否有效
                    if center is not None and mm_x is not None:
                        AXIS_X, AXIS_Y, ROT = mm_x, mm_y, rotation
                        DETECTED = True
                    else:
                        DETECTED = False
            ...
            if brightness_adjustment and all_qr_results:
                try:
                    ...
                    console.add_message(f"亮度调整失败: {str(e)}")
            # 创建显示图像
            display = frame.copy()
                    ...
                    for square in display_squares:
                        if square is not None and len(square) >= 4:
                            ...
                            center, mm_x, mm_y, mm_z, rotation = calculate_object_position(
                                square, frame.shape[1], frame.shape[0])
            ...
            # 更新坐标历史
            update_coordinate_history()

            # 显示主窗口
            # cv2.imshow("Multi-Square Detector", display)
            console.set_image(display)

            #使用同一个avg_calculator实例，而不是每次循环都创建新的
            processed_frame, real_world_coord = process_frame(frame, avg_calculator, console)
            # cv2.imshow("Processed 2", processed_frame)

            if real_world_coord is not None and current_qr_id is not None:
                # 将基于瓶盖的坐标转化为机械臂坐标
                OFFSET_X = 240
                OFFSET_Z = 60
                real_world_coord['z'] = (-real_world_coord['x'] + OFFSET_Z) / 10
                real_world_coord['x'] = (real_world_coord['y'] + OFFSET_X) / 10
                real_world_coord['qr_id'] = current_qr_id
                console.master.add_task(Message(MsgType.VISUAL_MODE, real_world_coord))

                # print(f"X: {real_world_coord['x']}, Z: {real_world_coord['z']}\n"
                #       f"ROT: {real_world_coord['rot']}, QR: {real_world_coord['qr_id']}")
                console.add_message(f"X: {real_world_coord['x']}, Z: {real_world_coord['z']}\n"
                       f"ROT: {real_world_coord['rot']}, QR: {real_world_coord['qr_id']}\n")
            ...
            # # 在按键处理部分添加
            # elif key == ord('r'):  # 按r键重置圆形检测
            #     reset_red_button_detection()
            # elif key == ord('z'):
