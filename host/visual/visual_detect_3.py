...
def find_red_button(frame, console : VisualConsole):
    ...
    if circles is not None:
        ...
        if len(circles) == 2:
            ...
            # print(f"检测到两个红色圆形，坐标已存储: {circles}")
            console.add_message(f"检测到两个红色圆形，坐标已存储: {circles}")
    else:
        console.add_message("未检测到红色圆形")
        # print("未检测到红色圆形")
...
def find_rotation(circles):
    ...
    dx = pt1[0] - pt0[0]
    dy = pt1[1] - pt0[1]
    rotation = -(np.degrees(abs(np.arctan2(dy, dx)))) % 180
...
def draw_line_with_angle(img, point, angle_degrees, length, color, thickness=1):
    """
    在图像上通过一个点和一个角度绘制直线
...
def process_frame(frame, avg_calculator, console : VisualConsole):
    ...
    # return processed_frame, circles
    ...
    #     x1 = (x11[0]+x11[1])//2
    #     y1 = (y11[0]+y11[1])//2
    #     rot1 = arctan(y11[1]-y11[0],x11[1]-x11[0])
    #     draw_line_with_angle(processed_frame,(int(x1),int(y1)),rot1,100,(255,255,255),5)
    #     cv2.line(processed_frame, (int(x11[0]), int(y11[0])), (int(x11[1]), int(y11[1])), (255, 255, 255), 5)
    ...
    if avg_calculator.update_averages():
        ...
        print(has_new_data)
        if has_new_data:
            processed_frame=draw_line_with_angle(processed_frame, (int(avg_x),int(avg_y)), avg_rotation, 100, (0, 255, 0))
            #print(f"\n=== 1秒平均值更新 ===")
...
def visual_detect_3(camera_id : int, console : VisualConsole):
    ...
    while DETECTED_CIRCLES is None:
        if not LOOP_FLAG:
            break
        ret, frame = cap.read()
        # console.set_image(frame)
        # continue
        if not ret:
            # logger.warning("无法获取帧")
            console.add_message("无法获取帧")
            break
        frame, _ = find_red_button(frame, console)
        console.set_image(frame)
    ...
    try:
        while True:
            ...
            avg_calculator.has_new_data = False
            ...
            # 修复：使用同一个avg_calculator实例，而不是每次循环都创建新的
            processed_frame, real_world_coord = process_frame(display, avg_calculator, console)
            # cv2.imshow("Processed 2", processed_frame)

            console.set_image(processed_frame)

            if has_detect_qr_code and real_world_coord is not None and current_qr_id is not None:
                # 将基于瓶盖的坐标转化为机械臂坐标
                OFFSET_X = 230
                OFFSET_Z = -5
