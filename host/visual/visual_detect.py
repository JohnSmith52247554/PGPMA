...
def update_coordinate_history(console : VisualConsole) -> dict | None:
    """更新坐标历史记录并计算平均值"""
        ...
        with COORD_LOCK:
            ...
            if COORDINATE_HISTORY and DETECTED:
                ...
                #print(f"[{time.strftime('%H:%M:%S')}] 平均坐标 - "
                      #f"X: {AXIS_X:.1f}mm, Y: {AXIS_Y:.1f}mm, 旋转: {ROT:.1f}° "
...
def visual_detect(camera_id : int, console : VisualConsole):
    ...
    console.add_message(f"NumPy版本:{np.__version__}")
    #print("OpenCV版本:", cv2.__version__)
    console.add_message(f"OpenCV版本:{cv2.__version__}")

    cap = cv2.VideoCapture(camera_id)
    if not cap.isOpened():
        #print("Error: Unable to open camera")
        console.add_message("Error: Unable to open camera")
        return

    #print("Multi-Square Detection Program Started (Center Coordinate System)...")
    console.add_message("Multi-Square Detection Program Started (Center Coordinate System)...")
    #print(f"Using GPU Acceleration: {CUDA_AVAILABLE}")
    #print("Press 's' to save detected squares")
    #print("Press 'c' to toggle contour display")
    #print("Press 'a' to toggle auto-save mode")
    #print("Press 'g' to toggle coordinate grid")
    #print("Press 't' to toggle processed images display")
    #print("Press 'd' to toggle debug mode")
    #print("Press 'z' to toggle zoom window")
    #print("Press 'l' to analyze last zoomed frame")
    ...
    if not ret:
        #print("Error: Unable to get first frame")
        console.add_message("Error: Unable to get first frame")
        return
    ...
    try:
        while True:
            ...
            if not ret:
                ...
                console.add_message("Error: Unable to get frame")
                break
            ...
            squares = squares_future.result()
            qr_results = qr_future.result()
            tilted_qr_results = tilted_qr_future.result()
            ...
            all_qr_results.sort(key=lambda x: x["id"], reverse=True)
            ...
            if ZOOM_ENABLED and not all_qr_results:
                ...
                if current_zoom_rect is not None:
                    try:
                        ...
                        logger.error(f"放大区域检索失败: {str(e)}")

                # 显示放大窗口
                if ZOOM_WINDOW_VISIBLE and current_zoom_rect is not None:
                    show_zoomed_window(frame, current_zoom_rect)

            # 合并接近的方形
            merged_squares = merge_close_squares(squares)
            ...
            if all_qr_results:
                ...
                # 检查是否应该保持之前的锁定
                keep_lock = should_keep_priority_lock(current_time)

                if keep_lock and priority_lock != current_qr_id:
                    # 保持之前的锁定，不处理当前检测结果
                    if last_valid_qr is not None:
                        display_qr = last_valid_qr["polygon"]  # 使用多边形顶点
                        if display_qr is not None:
                            center, AXIS_X, AXIS_Y, _, ROT = calculate_qr_position(
                                display_qr, frame.shape[1], frame.shape[2])
                            DETECTED = True
                else:
                    # 处理当前检测结果
                ...
                if display_qr is not None:
                    ...
                    DETECTED = True
            # elif merged_squares:
            #     # 如果没有QR码，但检测到矩形，则显示最大矩形
            #     display_squares = [max(merged_squares, key=cv2.contourArea)]
            #     center, AXIS_X, AXIS_Y, _, ROT = calculate_object_position(
            #         display_squares[0], frame.shape[1], frame.shape[0])
            #     DETECTED = True
            #     last_valid_square = display_squares[0]
            ...
            if frame_count >= 10:
                ...
                start_time = time.time()
                #print(f"FPS: {fps:.1f}")
                console.add_message(f"FPS: {fps:.1f}")
            ...
            if key == ord('q'):
                ...
                if display_squares:
                    ...
                    print(f"Saved {len(display_squares)} squares")
            elif key == ord('c'):
                ...
                if zoom_results:
                    print(f"Found {len(zoom_results)} QR codes in zoomed area")
            '''
